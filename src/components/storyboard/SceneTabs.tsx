...
interface SceneTabsProps {
  ...
  dragOverSceneForShot: number | null;
      ...
      {canScrollLeft && (
        <>
          ...
          <button
            ...
            className="touch-target-44-lg absolute left-2 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur border border-white/15 text-slate-300 hover:text-white flex items-center justify-center transition"
            title="向左滚动"
          >
      ...
      {canScrollRight && (
        <>
          ...
          <button
            ...
            className="touch-target-44-lg absolute right-2 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur border border-white/15 text-slate-300 hover:text-white flex items-center justify-center transition"
            title="向右滚动"
          >
        ...
        <div className="flex items-center gap-2 min-w-max px-1">
          {sortedScenes.map((scene) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center justify-center gap-1.5 py-1.5 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap border transition-all duration-200 ${
                ...
                } ${isDragOverScene || isShotDragOver ? 'ring-2 ring-violet-400/70 scale-105' : ''}`}
          ...
          {unclassifiedCount > 0 && (
            <button
              ...
              } ${dragOverSceneForShot === null ? 'ring-2 ring-violet-400/70 scale-105' : ''}`}
              title="未分类"
            >
              未分类
              {(() => {
          ...
          <button
            ...
            className="touch-target-44 flex items-center gap-1.5 px-2 sm:px-3 h-8 rounded-full border border-dashed border-white/25 hover:border-violet-400/50 hover:bg-violet-500/10 text-slate-400 hover:text-violet-200 transition shrink-0"
            title="场次管理"
          >
            <Plus className="w-4 h-4 shrink-0" />
            <span className="text-xs font-medium hidden sm:inline">新建及管理场次</span>
          </button>
        </div>
      </div>
    </div>
